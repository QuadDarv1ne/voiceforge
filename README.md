...
- 🎙️ **Выбор голоса Z.ai** — переключатель 7 голосов (tongtong, chuichui, xiaochen, jam, kazi, douji, luodo) прямо в интерфейсе вместо захардкоженного `tongtong`
- 🔄 **Replay из истории с учётом движка** — повторное воспроизведение записи пере-синтезирует через тот же движок (freetts / Z.ai / Piper), а для Web Speech подбирает голос под язык записи
- 🔁 **Повтор загрузки голосов Piper** — кнопка «Повторить», если локальный mini-service был недоступен при выборе движка
- 📁 **Корректные расширения файлов** — скачивание сохраняет `.mp3` для freetts и `.wav` для Z.ai/Piper вместо всегда `.mp3`
- 🏷️ **Метка движка в плеере** — корректно отображает «Piper (офлайн)» в аудиоплеере
- ⏸️ **Ведущая пауза SSML** — тег `<pause>` в начале текста теперь учитывается
- 🧹 **Очистка мёртвого кода** — удалён неиспользуемый `TAG_PATTERNS` из SSML-парсера
...
### 4 движка TTS в одном интерфейсе
...
| **Piper (офлайн)** | Локальный нейросетевой синтез, без интернета | 5000 символов | ✅ WAV | Высокое |
...
│   │
│   ├── components/
│   │   ├── engine-selector.tsx     # Переключатель 4 движков
...
Нажмите кнопку «Сравнить движки» — откроется диалог, где один и тот же текст озвучивается всеми 4 движками параллельно. Можно послушать каждый и скачать для сравнения качества.
